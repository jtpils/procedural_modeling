    ...
    def change_id(axial_id, mtg_id):
        ...
        if geoms:
            if geoms.has_key(axial_id):
                ...
                elif type(shape) is Translated:
                    if type(shape.geometry) is Oriented:
                        if type(shape.geometry.geometry) is Frustum:
                            mtg.property('XX')[mtg_id] = shape.translation.x
                            mtg.property('YY')[mtg_id] = shape.translation.y
                            mtg.property('ZZ')[mtg_id] = shape.translation.z
                            mtg.property('radius')[mtg_id] = shape.geometry.geometry.radius
                        else:  # type(shape.geometry.geometry) is Scaled
                            mtg.property('XX')[mtg_id] = shape.translation.x
                            mtg.property('YY')[mtg_id] = shape.translation.y
                            mtg.property('ZZ')[mtg_id] = shape.translation.z
                            mtg.property('radius')[mtg_id] = 0
                    else:  # type(shape.geometry) is Frustum
                        mtg.property('XX')[mtg_id] = shape.translation.x
                        mtg.property('YY')[mtg_id] = shape.translation.y
                        mtg.property('ZZ')[mtg_id] = shape.translation.z
    ...
    for aid, module in enumerate(tree):
        ...
        if label == '[':
            ...
            for p in _params:
                ...
                elif module.argSize() > 0:
                    try:
                        params[p] = module.args[ptr]
                    except:
                        pass
                ptr = ptr+1
